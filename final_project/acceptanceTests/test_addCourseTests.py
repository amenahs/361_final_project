...
class addProfCourseTests(TestCase):
    ...
    def test_noProf(self):
        ...
        self.assertEqual(resp.context["message"], "Must select an instructor.", msg="No professor selected did not result in error message")

    def test_noCourse(self):
        resp = self.client.post('/assigncourse-prof', {'instructor': 'inst@uwm.edu', 'course': ''})
        self.assertEqual(resp.context["message"], "Must select a course.", msg="No course selected did not result in error message")

    def test_validEntry(self):
        resp = self.client.post('/assigncourse-prof', {'instructor': 'inst@uwm.edu', 'course': 'CS 361'})
        self.assertEqual(resp.url, '/dashboard/', msg='User was not redirected to dashboard page after assigning course with valid input')
