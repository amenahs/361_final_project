...
class EditInformationUnitTests(TestCase):
    ...
    def test_invalidUser(self):
        with self.assertRaises(ValueError, msg="Non-existent user did not raise ValueError"):
            self.account.__editContactInfo__('idonotexist@uwm.edu', 'user', 'password', 1234567890, 'Milwaukee, WI', '')

    def test_validData_noChanges(self):
        c = self.account.__editContactInfo__('username@uwm.edu', 'name', '123', 123456790, 'Milwaukee, WI', '')
        self.assertFalse(c, msg="No changes made still resulted in data being re-saved")

    def test_validData_changeName(self):
        self.account.__editContactInfo__('username@uwm.edu', self.newName, '123', 1234567890, 'Milwaukee, WI', '')
    ...
    def test_taAddSkills(self):
        self.account.__editContactInfo__('newta@uwm.edu', "TA", 'abcd', 1234567890, 'Milwaukee, WI', self.taSkills)
