...
class AssignSectionUnitTests(TestCase):
    def setUp(self):
        ...
        self.course = Course.objects.create(courseID=361, name='Introduction to Software Engineering', numLectures=2, numSections=3)
    ...
    def test_assignSec_unassignedLec(self):
        with self.assertRaises(SyntaxError, msg="Admin assigning section with no TA-lecture assignment did not raise SyntaxError"):
            a = self.admin.__assignTASection__(self.ta.email, self.unassignedSec.sectionID)
